...
import { ConcurrentTask, type Task, intro, nextSteps, runTasks, runTasksConcurrently } from "../utils/prompts";
...
const _add = async (blockNames: string[], options: Options) => {
	...
	const resolvedRepos: gitProviders.ResolvedRepo[] = (await gitProviders.resolvePaths(...repoPaths)).match(
		(val) => val,
		({ repo, message }) => {
	...
	const blocksMap: Map<string, RemoteBlock> = (await gitProviders.fetchBlocks(...resolvedRepos)).match(
		(val) => val,
		({ repo, message }) => {
	...
	const installedBlocks = getInstalled(blocksMap, config, options.cwd).map((val) => val.specifier);
	...
	const installingBlocks = (await resolveTree(installingBlockNames, blocksMap, resolvedRepos)).match(
		(val) => val,
